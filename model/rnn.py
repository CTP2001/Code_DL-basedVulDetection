...
def build_model(maxlen, vector_dim, layers, dropout):
    ...
    model.add(SimpleRNN(units=256, activation='tanh'))
    model.add(Dropout(dropout))
    
    model.add(Dense(1, activation='sigmoid'))
          
    model.compile(loss='binary_crossentropy', optimizer='adamax', metrics=['TP_count', 'FP_count', 'FN_count', 'precision', 'recall', 'fbeta_score', 'accuracy'])
...
def main(traindataSet_path, testdataSet_path, weightpath, resultpath, batch_size, maxlen, vector_dim, layers, dropout):
    ...
    t2 = time.time()
    test_time = t2 - t1
    score, TP, FP, FN, precision, recall, f_score, acc = result[0]
    f = open("TP_index_rnn.pkl",'wb')
    pickle.dump(result[1], f)
    f.close()

    f_TP = open("./result_analyze/RNN/TP_filenames.txt","a+")
    f_TP.write(tname+'\n')
    ...
    f_FP = open("./result_analyze/RNN/FP_filenames.txt","a+")
    f_FP.write(tname+'\n')
    ...
    f_FN = open("./result_analyze/RNN/FN_filenames.txt","a+")
    f_FN.write(tname+'\n')
...
if __name__ == "__main__":
    ...
    weightPath = './model/RNN'
    resultPath = "./result/RNN/RNN"
